...
const VideoPlayer = ({
  ...
  const [showVideoOverlay, setShowVideoOverlay] = useState(
    autoPlay && (title || client || leftReelText || rightReelText)
  );
  ...
  useEffect(() => {
    const toggleVideoOverlay = async () => {
      ...
      const hasLeftAndRightText = leftReelText || rightReelText;
      const isPausedOrInitial = !hasClicked || isPaused;
      if ((hasTitleOrClient || hasLeftAndRightText) && isPausedOrInitial) {
    ...
    >
      <div className={playerState === "initial" ? "block" : "hidden"}>
        ...
        <VideoPlayerOverlayButton
          ...
          title={title}
          leftReelText={leftReelText}
          rightReelText={rightReelText}
        />
      </div>
